...
class AvatarNet(nn.Module):
    def __init__(self, opt, rank: int = 0):
        ...
        cano_smpl_map = cv.imread(
            config.opt["train"]["data"]["data_dir"]
            + "/smpl_pos_map/cano_smpl_pos_map.exr",
            cv.IMREAD_UNCHANGED,
        )
        self.cano_smpl_map = (
        ...
        if self.with_viewdirs:
            cano_nml_map = cv.imread(
                config.opt["train"]["data"]["data_dir"]
                + "/smpl_pos_map/cano_smpl_nml_map.exr",
                cv.IMREAD_UNCHANGED,
            )
            self.cano_nml_map = (
    ...
    def generate_mean_hands(self):
        ...
        pose_map_paths = sorted(
            glob.glob(
                config.opt["train"]["data"]["data_dir"]
                + "/smpl_pos_map/%08d.exr" % config.opt["test"]["fix_hand_id"]
            )
        )
    ...
    def transform_cano2live(self, gaussian_vals, items):
        ...
        gaussian_vals["positions"] = (
            torch.einsum("nxy,ny->nx", pt_mats[..., :3, :3], gaussian_vals["positions"])
            + pt_mats[..., :3, 3]
        )
    ...
    def get_positions(self, pose_map, return_map=False):
        position_map, _ = self.position_net(
            [self.position_style], pose_map[None], randomize_noise=False
        )
        front_position_map, back_position_map = torch.split(position_map, [3, 3], 1)
        position_map = torch.cat([front_position_map, back_position_map], 3)[0].permute(
            1, 2, 0
        )
    ...
    def get_others(self, pose_map):
        other_map, _ = self.other_net(
            [self.other_style], pose_map[None], randomize_noise=False
        )
        ...
        opacity = self.cano_gaussian_model.opacity_activation(
            opacity + self.cano_gaussian_model.get_opacity_raw
        )
        scales = self.cano_gaussian_model.scaling_activation(
            scales + self.cano_gaussian_model.get_scaling_raw
        )
        rotations = self.cano_gaussian_model.rotation_activation(
            rotations + self.cano_gaussian_model.get_rotation_raw
        )
    ...
    def get_colors(self, pose_map, front_viewdirs=None, back_viewdirs=None):
        color_style = (
            torch.rand_like(self.color_style)
            if self.random_style and self.training
            else self.color_style
        )
        color_map, _ = self.color_net(
            [color_style],
            pose_map[None],
    ...
    def get_viewdir_feat(self, items):
        with torch.no_grad():
            pt_mats = torch.einsum("nj,jxy->nxy", self.lbs, items["cano2live_jnt_mats"])
            live_pts = (
                torch.einsum("nxy,ny->nx", pt_mats[..., :3, :3], self.init_points)
                + pt_mats[..., :3, 3]
            )
            live_nmls = torch.einsum("nxy,ny->nx", pt_mats[..., :3, :3], self.cano_nmls)
            cam_pos = -torch.matmul(
                torch.linalg.inv(items["extr"][:3, :3]), items["extr"][:3, 3]
            )
        ...
        front_viewdirs = self.opt.get("weight_viewdirs", 1.0) * self.viewdir_net(
            front_viewdirs
        )
        back_viewdirs = self.opt.get("weight_viewdirs", 1.0) * self.viewdir_net(
            back_viewdirs
        )
        return front_viewdirs, back_viewdirs

    def get_pose_map(self, items):
        pt_mats = torch.einsum(
            "nj,jxy->nxy", self.lbs, items["cano2live_jnt_mats_woRoot"]
        )
        live_pts = (
            torch.einsum("nxy,ny->nx", pt_mats[..., :3, :3], self.init_points)
            + pt_mats[..., :3, 3]
        ...
        live_pos_map = F.interpolate(
            live_pos_map.permute(2, 0, 1)[None], None, [0.5, 0.5], mode="nearest"
        )[0]
        live_pos_map = torch.cat(torch.split(live_pos_map, [512, 512], 2), 0)
        items.update({"smpl_pos_map": live_pos_map})
        return live_pos_map

    # The render method actually contains the forward pass of the network
    def render(self, items, bg_color=(0.0, 0.0, 0.0), use_pca=False, use_vae=False):
        """
        ...
        if (
            not self.training
            and config.opt["test"].get("fix_hand", False)
            and config.opt["mode"] == "test"
        ):
            # print('# fuse hands ...')
            ...
            wl = torch.sigmoid(
                ...
                * (
                    geo_util.normalize_vert_bbox(
                        items["left_cano_mano_v"], attris=cano_xyz, dim=0, per_axis=True
                    )[..., 0:1]
                    + 2.0
                )
            )
            ...
            wl[cano_xyz[..., 1] < items["cano_smpl_center"][1]] = 0.0
            wr[cano_xyz[..., 1] < items["cano_smpl_center"][1]] = 0.0
        ...
        if not self.training:
            ret.update({"cano_tex_map": color_map, "posed_gaussians": gaussian_vals})
