...
def save_pos_map(pos_map, path):
    mask = np.linalg.norm(pos_map, axis=-1) > 0.0
...
if __name__ == "__main__":
    ...
    MvRgbDataset = importlib.import_module("dataset.dataset_mv_rgb").__getattribute__(
        dataset_module
    )
    ...
    smpl_model = smplx.SMPLX(
        config.PROJ_DIR + "/smpl_files/smplx",
        gender="neutral",
        use_pca=False,
        num_pca_comps=45,
        flat_hand_mean=True,
    ...
    with torch.no_grad():
        # Infer the canonical SMPL model
    ...
    body_mask = np.linalg.norm(cano_pos_map, axis=-1) > 0.0
    cano_pts = cano_pos_map[body_mask]
    if using_template:
        weight_volume = CanoBlendWeightVolume(data_dir + "/cano_weight_volume.npz")
        pts_lbs = weight_volume.forward_weight(torch.from_numpy(cano_pts)[None].cuda())[
            0
        ]
    else:
        pts_lbs = interpolate_lbs(
            cano_pts, cano_smpl_v, smpl_faces, smpl_model.lbs_weights
        )
        pts_lbs = torch.from_numpy(pts_lbs).cuda()
    np.save(data_dir + "/smpl_pos_map/init_pts_lbs.npy", pts_lbs.cpu().numpy())

    # Compute positional maps
    ...
    for pose_idx in tqdm.tqdm(frame_list, desc="Generating positional maps..."):
        ...
        cano2live_jnt_mats_woRoot = torch.matmul(
            live_smpl_woRoot.A.cuda(), inv_cano_smpl_A
        )[0]
        pt_mats = torch.einsum("nj,jxy->nxy", pts_lbs, cano2live_jnt_mats_woRoot)
        live_pts = (
            torch.einsum("nxy,ny->nx", pt_mats[..., :3, :3], cano_pts)
            + pt_mats[..., :3, 3]
        )
