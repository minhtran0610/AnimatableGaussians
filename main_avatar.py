...
class AvatarTrainer:
    def __init__(self, opt):
        ...
        print(
            "# Parameter number of AvatarNet is %d"
            % (sum([p.numel() for p in self.avatar_net.parameters()]))
        )
    ...
    def crop_image(self, gt_mask, patch_size, randomly, *args):
        ...
        """
        mask_uv = torch.argwhere(gt_mask > 0.0)
        ...
        for image in args:
            cropped_image = self.bg_color_cuda[:, None, None] * torch.ones(
                (3, max_size, max_size), dtype=image.dtype, device=image.device
            )
            if len_v > len_u:
                start_u = (max_size - len_u) // 2
                cropped_image[:, :, start_u : start_u + len_u] = image[
                    :, min_v:max_v, min_u:max_u
                ]
            else:
                start_v = (max_size - len_v) // 2
                cropped_image[:, start_v : start_v + len_v, :] = image[
                    :, min_v:max_v, min_u:max_u
                ]
            ...
            if randomly and max_size > patch_size:
                cropped_image = cropped_image[
                    :,
                    random_v : random_v + patch_size,
                    random_u : random_u + patch_size,
                ]
            else:
                cropped_image = F.interpolate(
                    cropped_image[None], size=(patch_size, patch_size), mode="bilinear"
                )[0]
    ...
    def compute_lpips_loss(self, image, gt_image):
        assert (
            image.shape[1] == image.shape[2] and gt_image.shape[1] == gt_image.shape[2]
        )
    ...
    def forward_one_pass_pretrain(self, items, ddp_avatar_net, ddp_optimizer):
        ...
        # Pretrain the models: position, opacity, scale, rotation
    ...
    def forward_one_pass(self, items, ddp_avatar_net, ddp_optimizer):
        ...
        # Select a random background color if specified
        ...
        boundary_mask_img = 1.0 - items["boundary_mask_img"].to(torch.float32)
        image = (
            image * boundary_mask_img[None]
            + (1.0 - boundary_mask_img[None]) * self.bg_color_cuda[:, None, None]
        )
        gt_image = (
            gt_image * boundary_mask_img[None]
            + (1.0 - boundary_mask_img[None]) * self.bg_color_cuda[:, None, None]
        )
        ...
        if self.loss_weight["l1"] > 0.0:
            l1_loss = torch.abs(image - gt_image).mean()
            total_loss += self.loss_weight["l1"] * l1_loss
            batch_losses.update({"l1_loss": l1_loss.item()})

        if self.loss_weight.get("mask", 0.0) and "mask_map" in render_output:
            ...
            total_loss += self.loss_weight.get("mask", 0.0) * mask_loss
            batch_losses.update({"mask_loss": mask_loss.item()})

        if self.loss_weight["lpips"] > 0.0:
            # crop images
            random_patch_flag = False if self.iter_idx < 300000 else True
            image, gt_image = self.crop_image(
                mask_img, self.patch_size, random_patch_flag, image, gt_image
            )
            # cv.imshow('image', image.detach().permute(1, 2, 0).cpu().numpy())
            # cv.imshow('gt_image', gt_image.permute(1, 2, 0).cpu().numpy())
            # cv.waitKey(0)
            ...
            batch_losses.update({"lpips_loss": lpips_loss.item()})
        ...
        if True:
            ...
            batch_losses.update({"offset_loss": offset_loss.item()})
    ...
    def pretrain(self, rank, world_size):
        ...
        ).__getattribute__(dataset_module)
        ...
        log_dir = (
            ...
            + "/"
            + datetime.datetime.now().strftime("pretrain_%Y_%m_%d_%H_%M_%S")
        )
        ...
        for epoch_idx in range(0, 9999999):
            ...
            for batch_idx, items in enumerate(dataloader):
                ...
                if self.iter_idx % smooth_interval == 0:
                    log_info = "epoch %d, batch %d, iter %d, " % (
                        epoch_idx,
                        batch_idx,
                        self.iter_idx,
                    )
                    for key in smooth_losses.keys():
                        ...
                        writer.add_scalar(
                            "%s/Iter" % key, smooth_losses[key], self.iter_idx
                        )
                        log_info = log_info + ("%s: %f, " % (key, smooth_losses[key]))
                        smooth_losses[key] = 0.0
                    smooth_count = 0
                    print(log_info)
    ...
    def train(self, rank, world_size):
        ...
        if self.opt["train"]["prev_ckpt"] is not None:
            start_epoch, self.iter_idx = self.load_ckpt(
                self.opt["train"]["prev_ckpt"], load_optm=True
            )
            start_epoch += 1
            self.iter_idx += 1
        else:
            ...
            if safe_exists(prev_ckpt_path):
                ...
                if safe_exists(self.opt["train"]["pretrained_dir"]):
                    ...
                    self.load_ckpt(
                        self.opt["train"]["net_ckpt_dir"] + "/pretrained",
                        load_optm=False,
                    )
                else:
        ...
        for epoch_idx in range(start_epoch, 9999999):
            ...
            for batch_idx, items in enumerate(dataloader):
                ...
                if self.iter_idx % smooth_interval == 0:
                    log_info = "epoch %d, batch %d, iter %d, lr %e, " % (
                        epoch_idx,
                        batch_idx,
                        self.iter_idx,
                        lr,
                ...
                if (
                    self.iter_idx % self.opt["train"]["eval_interval"] == 0
                    and self.iter_idx != 0
                ):
                ...
                if (
                    ...
                    model_folder = (
                        self.opt["train"]["net_ckpt_dir"] + "/batch_%d" % self.iter_idx
                    )
                    os.makedirs(model_folder, exist_ok=True)
                    self.save_ckpt(model_folder, save_optm=True)
            ...
            """ End of epoch """
            if (
                epoch_idx % self.opt["train"]["ckpt_interval"]["epoch"] == 0
                and epoch_idx != 0
            ):
                model_folder = (
    ...
    def test(self):
        ...
        # If no pose is specified, test on test split of the training dataset
            ...
            self.opt["test"][
                "n_pca"
            ] = -1  # cancel PCA for training pose reconstruction
        ...
        if output_dir is None:
            ...
            output_dir = (
                f"./test_results/{training_dataset.subject_name}/{exp_name}/{dataset_name}_{seq_name}_{view_folder}"
                + "/batch_%06d" % iter_idx
            )

        use_pca = self.opt["test"].get("n_pca", -1) >= 1
        if use_pca:
            output_dir += "/pca_%d_sigma_%.2f" % (
                self.opt["test"].get("n_pca", -1),
                float(self.opt["test"].get("sigma_pca", 1.0)),
            )
        else:
            output_dir += "/vanilla"
        ...
        global_orient = (
            item_0["global_orient"].cpu().numpy()
            if isinstance(item_0["global_orient"], torch.Tensor)
            else item_0["global_orient"]
        )
        ...
        # For each frame in the testing sequence
        for idx in tqdm(range(data_num), desc="Rendering avatars..."):
            ...
            # Multiple options for viewpoint setting, very powerful
            # According to each setting, we will create a specific set of camera
            # parameters and render the image
            if view_setting == "camera":
                # training view setting
                cam_id = config.opt["test"]["render_view_idx"]
                # render_view_idx: Index of a particular camera
                # Get the camera parameters
                intr = self.dataset.intr_mats[cam_id].copy()
                intr[:2] *= img_scale
                extr = self.dataset.extr_mats[cam_id].copy()
                img_h, img_w = int(self.dataset.img_heights[cam_id] * img_scale), int(
                    self.dataset.img_widths[cam_id] * img_scale
                )
            elif view_setting.startswith("free"):
                # free view setting
                # rotation with free view setting
                # In the future: Ability to render with any arbitrary viewpoint
                # frame_num_per_circle = 360
                frame_num_per_circle = 216
                rot_Y = (
                    (idx % frame_num_per_circle)
                    / float(frame_num_per_circle)
                    * 2
                    * np.pi
                )

                extr = visualize_util.calc_free_mv(
                    object_center,
                    tar_pos=np.array([0, 0, 2.5]),
                    rot_Y=rot_Y,
                    rot_X=0.3 if view_setting.endswith("bird") else 0.0,
                    global_orient=(
                        global_orient
                        if self.opt["test"].get("global_orient", False)
                        else None
                    ),
                )
                ...
                extr = visualize_util.calc_free_mv(
                    ...
                    tar_pos=np.array([0, 0, 2.5]),
                    rot_Y=np.pi,
                    rot_X=0.5 * np.pi / 4.0 if view_setting.endswith("bird") else 0.0,
                    global_orient=(
                        global_orient
                        if self.opt["test"].get("global_orient", False)
                        else None
                ...
                extr[2, 3] += f_len / 512
                intr = np.array(
                    [[f_len, 0, 512], [0, f_len, 512], [0, 0, 1]], np.float32
                )
            ...
            # Selec the method to load data
            getitem_func = (
                self.dataset.getitem_fast
                if hasattr(self.dataset, "getitem_fast")
                else self.dataset.getitem
            )
            item = getitem_func(
                idx, training=False, extr=extr, intr=intr, img_w=img_w, img_h=img_h
            )
            items = to_cuda(item, add_batch=False)

            # When the view is moving, the object center changes throughout time
            ...
            if log_time:
                ...
                torch.cuda.synchronize()
                print(
                    "Loading data costs %.4f secs"
                    % (time_start.elapsed_time(time_end) / 1000.0)
                )
                time_start.record()

            # Whether to render the skeleton
            if self.opt["test"].get("render_skeleton", False):
                from utils.visualize_skeletons import construct_skeletons

                skel_vertices, skel_faces = construct_skeletons(
                    item["joints"].cpu().numpy(), item["kin_parent"].cpu().numpy()
                )
                skel_mesh = trimesh.Trimesh(skel_vertices, skel_faces, process=False)

                if geo_renderer is None:
                    geo_renderer = Renderer(
                        item["img_w"],
                        item["img_h"],
                        shader_name="phong_geometry",
                        bg_color=(1, 1, 1),
                    )
                extr, intr = item["extr"], item["intr"]
                geo_renderer.set_camera(extr, intr)
                geo_renderer.set_model(
                    skel_vertices[skel_faces.reshape(-1)],
                    skel_mesh.vertex_normals.astype(np.float32)[skel_faces.reshape(-1)],
                )
                skel_img = geo_renderer.render()[:, :, :3]
                skel_img = (skel_img * 255).astype(np.uint8)
                cv.imwrite(
                    output_dir + "/live_skeleton/%08d.jpg" % item["data_idx"], skel_img
                )
            ...
            if log_time:
                ...
                torch.cuda.synchronize()
                print(
                    "Rendering skeletons costs %.4f secs"
                    % (time_start.elapsed_time(time_end) / 1000.0)
                )
                time_start.record()
            ...
            if use_pca:
                ...
                new_pose_conds = training_dataset.transform_pca(
                    pose_conds, sigma_pca=float(self.opt["test"].get("sigma_pca", 2.0))
                )
                front_live_pos_map[mask] = new_pose_conds
                live_pos_map = np.concatenate(
                    [front_live_pos_map, back_live_pos_map], 2
                )
                items.update(
                    {
                        "smpl_pos_map_pca": torch.from_numpy(live_pos_map)
                        .to(config.device)
                        .permute(2, 0, 1)
                    }
                )
            ...
            if log_time:
                ...
                torch.cuda.synchronize()
                print(
                    "Rendering pose conditions costs %.4f secs"
                    % (time_start.elapsed_time(time_end) / 1000.0)
                )
                time_start.record()

            # Actual rendering process
            output = self.avatar_net.render(
                items, bg_color=self.bg_color, use_pca=use_pca
            )
            if log_time:
                time_end.record()
                torch.cuda.synchronize()
                print(
                    "Rendering avatar costs %.4f secs"
                    % (time_start.elapsed_time(time_end) / 1000.0)
                )
                time_start.record()
            ...
            if "mask_map" in output:
                ...
                mask_map.clip_(0.0, 1.0)
                mask_map = (mask_map * 255).to(torch.uint8)
                cv.imwrite(
                    output_dir + "/mask_map/%08d.png" % item["data_idx"],
                    mask_map.cpu().numpy(),
                )
            ...
            if self.opt["test"].get("save_tex_map", False):
                ...
                cano_tex_map.clip_(0.0, 1.0)
                cano_tex_map = (cano_tex_map * 255).to(torch.uint8)
                cv.imwrite(
                    output_dir + "/cano_tex_map/%08d.jpg" % item["data_idx"],
                    cano_tex_map.cpu().numpy(),
                )

            if self.opt["test"].get("save_ply", False):
                save_gaussians_as_ply(
                    output_dir + "/posed_gaussians/%08d.ply" % item["data_idx"],
                    output["posed_gaussians"],
                )
            ...
            if log_time:
                ...
                torch.cuda.synchronize()
                print(
                    "Saving images costs %.4f secs"
                    % (time_start.elapsed_time(time_end) / 1000.0)
                )
                print(
                    "Animating one frame costs %.4f secs"
                    % (time_start_all.elapsed_time(time_end) / 1000.0)
                )
    ...
    def load_ckpt(self, path, load_optm=True):
        ...
        if load_optm and os.path.exists(path + "/optm.pt"):
            ...
            if "avatar_net" in optm_dict:
                ...
                print(
                    '[WARNING] Cannot find "avatar_net" from the optimizer checkpoint!'
                )
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        "-m", "--mode", type=str, help="Running mode.", default="train"
    )
    ...
    if config.opt["mode"] == "train":
        if (
            not safe_exists(config.opt["train"]["net_ckpt_dir"] + "/pretrained")
