...
def get_rays(uv, extr, intr):
    ...
    world_coords = (
        np.einsum("ij,nj->ni", inv_extr[:3, :3], pixel_points_cam) + inv_extr[:3, 3]
    )
...
def sample_pts_on_rays(
    ray_o, ray_d, near, far, N_samples=64, perturb=False, depth_guided_mask=None
):
    # calculate the steps for each ray
    t_vals = torch.linspace(0.0, 1.0, steps=N_samples).to(near)
    ...
    if perturb:
        ...
        if depth_guided_mask is None:
            depth_guided_mask = torch.ones(
                ray_o.shape[:-1], dtype=torch.bool, device=ray_o.device
            )
        ...
        mids = 0.5 * (z_vals[depth_guided_mask, 1:] + z_vals[depth_guided_mask, :-1])
...
def sample_pdf(bins, weights, N_samples, det=False, pytest=False):
    ...
    if det:
        u = torch.linspace(0.0, 1.0, steps=N_samples).to(weights)
...
def sample_pts_on_rays_fine(rays_o, rays_d, z_vals, weights, N_importance, perturb=0.0):
...
def raw2outputs(raw, z_vals, white_bkgd=False):
    ...
    disp_map = 1.0 / torch.max(
        1e-10 * torch.ones_like(depth_map).to(depth_map),
...
def sample_randomly_for_nerf_rendering(
    color_img,
    mask_img,
    depth_img,
    extr,
    ...
    assert (
        color_img.shape[:2] == mask_img.shape[:2]
        and color_img.shape[:2] == depth_img.shape[:2]
    )
    assert 0.0 <= inside_radio <= 1.0
    ...
    while count < sample_num:
        ...
        sampled_inside_uv = inside_uv[
            np.random.choice(inside_uv.shape[0], inside_sample_num, replace=False)
        ]
        sampled_outside_uv = outside_uv[
            np.random.choice(outside_uv.shape[0], outside_sample_num, replace=False)
        ]
...
def sample_patch_for_nerf_rendering(
    color_img,
    mask_img,
    depth_img,
    extr,
    ...
    if resize_factor != 1.0:
        ...
        if unsample_region_mask is not None:
            unsample_region_mask = (
                cv2.resize(
                    unsample_region_mask.astype(np.uint8),
                    (0, 0),
                    fx=resize_factor,
                    fy=resize_factor,
                    interpolation=cv2.INTER_NEAREST,
    ...
    img_h, img_w = color_img.shape[:2]
    bound_mask = get_bound_2d_mask(live_bounds, intr, extr, img_h, img_w) > 0.0
    ...
    for patch_idx in range(patch_num):
        ...
        center_uv = candidate_uv[
            np.random.choice(candidate_uv.shape[0], 1, replace=False)[0]
        ]
        ...
        near_patch, far_patch, mask_at_bound = get_near_far(
            live_bounds, ray_o_patch, ray_d_patch
        )
...
def sample_randomly_for_nerf_rendering_wSideViewMask(
    ...
    if unsample_region_mask is not None:
        bound_mask = np.logical_and(bound_mask, unsample_region_mask < 1e-6)
    uv_img = gen_uv(img_w, img_h)
    inside_uv_sideView = uv_img[
        np.logical_and(np.logical_and(mask_img, bound_mask), side_view_mask)
    ]
    inside_uv_fbView = uv_img[
        np.logical_and(np.logical_and(mask_img, bound_mask), ~side_view_mask)
    ]
    ...
    while count < sample_num:
        ...
        inside_sample_num_2 = int(rest_num * inside_radio * (1.0 - side_view_radio))
        outside_sample_num = rest_num - (inside_sample_num_1 + inside_sample_num_2)
        sampled_inside_uv_1 = inside_uv_sideView[
            np.random.choice(
                inside_uv_sideView.shape[0],
                inside_sample_num_1,
                replace=inside_sample_num_1 < inside_uv_sideView.shape[0],
            )
        ]
        sampled_inside_uv_2 = inside_uv_fbView[
            np.random.choice(
                inside_uv_fbView.shape[0],
                inside_sample_num_2,
                replace=inside_sample_num_2 < inside_uv_fbView.shape[0],
            )
        ]
        sampled_outside_uv = outside_uv[
            np.random.choice(outside_uv.shape[0], outside_sample_num, replace=False)
        ...
        uv_ = np.concatenate(
            [sampled_inside_uv_1, sampled_inside_uv_2, sampled_outside_uv], axis=0
        )
        ray_d_, ray_o_ = get_rays(uv_, extr, intr)
        near_, far_, mask_at_bound = get_near_far(live_bounds, ray_o_, ray_d_)
        uv.append(uv_[mask_at_bound])
