...
class PoseDataset(Dataset):
    ...
    def __init__(
        ...
        # Read the weights in .npz files and the calibration info
        data_name, ext = os.path.splitext(os.path.basename(data_path))
        if ext == ".pkl":
            smpl_data = pickle.load(open(data_path, "rb"))
            smpl_data = dict(smpl_data)
            self.body_poses = torch.from_numpy(smpl_data["smpl_poses"]).to(
                torch.float32
            )
            self.transl = (
                torch.from_numpy(smpl_data["smpl_trans"]).to(torch.float32) * 1e-3
            )
            ...
            if (
                ...
                or self.dataset_name == "actorshq"
                or self.dataset_name == "avatarrex"
            ):
                ...
                smpl_data["body_pose"][:, 19 * 3 : 20 * 3] = 0.0
                smpl_data["body_pose"][:, 20 * 3 : 21 * 3] = 0.0
                smpl_data["body_pose"][:, 14 * 3] = 0.0

            if self.seq_name == "_actor01":
                smpl_data["body_pose"][:, 6 * 3 : 7 * 3] = 0.0
                smpl_data["body_pose"][:, 7 * 3 : 8 * 3] = 0.0

            smpl_data = {
                k: torch.from_numpy(v).to(torch.float32) for k, v in smpl_data.items()
            }
            ...
            if os.path.exists(calib_path):
                ...
                for view_idx in range(self.view_num):
                    ...
                    extr_mat[:3, :3] = np.array(
                        cam_data[self.cam_names[view_idx]]["R"], np.float32
                    ).reshape(3, 3)
                    extr_mat[:3, 3] = np.array(
                        cam_data[self.cam_names[view_idx]]["T"], np.float32
                    )
                    self.extr_mats.append(extr_mat)
                self.intr_mats = [
                    np.array(
                        cam_data[self.cam_names[view_idx]]["K"], np.float32
                    ).reshape(3, 3)
                    for view_idx in range(self.view_num)
                ]
                self.img_heights = [
                    cam_data[self.cam_names[view_idx]]["imgSize"][1]
                    for view_idx in range(self.view_num)
                ]
                self.img_widths = [
                    cam_data[self.cam_names[view_idx]]["imgSize"][0]
                    for view_idx in range(self.view_num)
                ]
        else:
            raise AssertionError("Invalid data_path!")

        # Body pose info and SMPL model
        if "left_hand_pose" in smpl_data:
            self.left_hand_pose = smpl_data["left_hand_pose"]
        else:
            self.left_hand_pose = config.left_hand_pose[None].expand(
                self.body_poses.shape[0], -1
            )
        if "right_hand_pose" in smpl_data:
            self.right_hand_pose = smpl_data["right_hand_pose"]
        else:
            self.right_hand_pose = config.right_hand_pose[None].expand(
                self.body_poses.shape[0], -1
            )
        ...
        # Extract the frames specified by frame_range
                    ...
                    for interval in frame_range:
                        if len(interval) == 2 or len(interval) == 3:
                            ...
                            for i in range(interval[3]):
                                self.pose_list += list(
                                    range(interval[0], interval[1], interval[2])
                                )
                else:
                    if len(frame_range) == 2:
                        print(
                            f"# Selected frame indices: range({frame_range[0]}, {frame_range[1]})"
                        )
                        frame_range = range(frame_range[0], frame_range[1])
                    elif len(frame_range) == 3:
                        print(
                            f"# Selected frame indices: range({frame_range[0]}, {frame_range[1]}, {frame_range[2]})"
                        )
                        frame_range = range(
                            frame_range[0], frame_range[1], frame_range[2]
                        )
        ...
        # SMPL related
        self.smpl_shape = (
            smpl_shape.to(torch.float32).to(device)
            if smpl_shape is not None
            else torch.zeros(10, dtype=torch.float32)
        )
        # Canonical SMPL model
        ret = self.smpl_model.forward(
            betas=self.smpl_shape[None],
        ...
        max_xyz[2] += 0.15
        self.cano_bounds = (
            torch.stack([min_xyz, max_xyz], 0).to(torch.float32).cpu().numpy()
        )
        ...
        if self.denoise:
            ...
            for i in range(-win_size, win_size + 1):
                self.body_poses[win_size : frame_num - win_size] += body_poses_clone[
                    win_size + i : frame_num - win_size + i
                ]
                self.transl[win_size : frame_num - win_size] += transl_clone[
                    win_size + i : frame_num - win_size + i
                ]
        ...
        (
            self.left_cano_mano_v,
            self.left_cano_mano_n,
            self.right_cano_mano_v,
            self.right_cano_mano_n,
    ...
    def getitem(self, index, **kwargs):
        ...
        # Predefined hand pose from the config file
        if self.hand_pose_type == "fist":
            ...
            left_hand_pose[:3] = 0.0
            right_hand_pose[:3] = 0.0
        # Predefined normal hand pose
            ...
            left_hand_pose = torch.tensor(
                [
                    ...
                    -0.4484838545322418,
                    -0.11360692232847214,
                    -0.023141659796237946,
                    0.10571160167455673,
                    -0.08793719857931137,
                    -0.026760095730423927,
                    -0.41390693187713623,
                    -0.0923849567770958,
                    0.10266668349504471,
                    -0.36039748787879944,
            ...
            right_hand_pose = torch.tensor(
                [
                    ...
                    0.2961004376411438,
                    -0.07659692317247391,
                    0.004730052314698696,
                    -0.12084470689296722,
                    0.007974660955369473,
                    0.05222926288843155,
                    0.32775357365608215,
                    -0.10166633129119873,
                    -0.06862349808216095,
                    0.174485981464386,
                    -0.0023323255591094494,
                    0.04998664930462837,
                    -0.03490559384226799,
                    0.12949667870998383,
                    0.26883721351623535,
        ...
        # SMPL
        # Obtain the SMPL model with the given body pose and hand pose
        # Parameters of the SMPL model: Obtained in the .npz file
        # The SMPL shape is default to be zeros
        # Hand pose: obtained from the step above
        ...
        # Another forward pass to obtain the 3D vertices and joint transformations of
        # the body pose
        ...
        data_item = dict()
        # SMPL information saved in the .npy file.
        # Each frame has its own set of SMPL information
        ...
        if self.frame_win > 0:
            ...
            selected_frames = self.pose_list[
                max(0, index - self.frame_win) : min(
                    total_frame_num, index + self.frame_win + 1
                )
            ]
        ...
        if self.fix_head_pose:
            data_item["pose"][..., 3 * 11 : 3 * 11 + 3] = 0.0
            data_item["pose"][..., 3 * 14 : 3 * 14 + 3] = 0.0
        if self.fix_hand_pose:
            data_item["pose"][..., 3 * 19 : 3 * 19 + 3] = 0.0
            data_item["pose"][..., 3 * 20 : 3 * 20 + 3] = 0.0
        ...
        # Vertices of the SMPL model on the current frame
        data_item["live_smpl_v"] = live_smpl.vertices[0]
        data_item["live_smpl_v_woRoot"] = live_smpl_woRoot.vertices[0]
        # Canonical frame info
        data_item["cano_smpl_v"] = self.cano_smpl["vertices"]
        data_item["cano_jnts"] = self.cano_smpl["joints"]
        # Transformation from canonical to live
        inv_cano_jnt_mats = torch.linalg.inv(self.cano_smpl["A"])
        data_item["cano2live_jnt_mats"] = torch.matmul(
            live_smpl.A[0], inv_cano_jnt_mats
        )
        data_item["cano2live_jnt_mats_woRoot"] = torch.matmul(
            live_smpl_woRoot.A[0], inv_cano_jnt_mats
        )
        # Bounds and center of the canonical SMPL model and the live SMPL model
        ...
        # Output frame configuration (resolution, etc.)
        """ synthesis config """
        img_h = 512 if "img_h" not in kwargs else kwargs["img_h"]
        img_w = 512 if "img_w" not in kwargs else kwargs["img_w"]
        intr = (
            np.array([[550, 0, 256], [0, 550, 256], [0, 0, 1]], np.float32)
            if "intr" not in kwargs
            else kwargs["intr"]
        )
        if "extr" not in kwargs:
            extr = visualize_util.calc_front_mv(
                live_bounds.mean(0), tar_pos=np.array([0, 0, 2.5])
            )
        else:
            extr = kwargs["extr"]
    ...
    def getitem_fast(self, index, **kwargs):
        ...
        (
            data_item["left_cano_mano_v"],
            data_item["left_cano_mano_n"],
            data_item["right_cano_mano_v"],
            data_item["right_cano_mano_n"],
        ) = (
            self.left_cano_mano_v,
            self.left_cano_mano_n,
            self.right_cano_mano_v,
        ...
        data_item.update({"img_h": img_h, "img_w": img_w, "extr": extr, "intr": intr})
    ...
    def getitem_a_pose(self, **kwargs):
        ...
        intr = (
            np.array([[550, 0, 150], [0, 550, 256], [0, 0, 1]], np.float32)
            if "intr" not in kwargs
            else kwargs["intr"]
        )
        if "extr" not in kwargs:
    ...
    def gen_uv(img_w, img_h):
        x, y = np.meshgrid(
            np.linspace(0, img_w - 1, img_w, dtype=int),
