...
class MvRgbDatasetBase(Dataset):
    ...
    def __init__(
        ...
        # Load the camera data and the SMPL data (should be all included in the raw dataset)
        self.load_cam_data()
        self.load_smpl_data()

        # Load the SMPL model (should be included in the raw dataset)
        ...
        # Get the images according to the frame range and camera view
        # in the config file
        # A pose mean a frame in sequence, and a view means the image from a specific
        # camera capturing that frame
        ...
        if frame_range is not None:
            if isinstance(frame_range, list):
                ...
                frame_range = (
                    np.loadtxt(self.data_dir + "/" + frame_range)
                    .astype(np.int)
                    .tolist()
                )
        ...
        # Define the canonical boundaries and the canonical faces
    ...
    # Each item originates from a pose and a camera view
    def getitem(self, index, training=True, **kwargs):
        # Frame and camera view
        # During testing, generate new poses -> no view
        ...
        if self.load_smpl_pos_map:
            smpl_pos_map = cv.imread(
                self.data_dir + "/smpl_pos_map/%08d.exr" % pose_idx, cv.IMREAD_UNCHANGED
            )
            pos_map_size = smpl_pos_map.shape[1] // 2
            smpl_pos_map = np.concatenate(
        ...
        if self.load_smpl_nml_map:
            smpl_nml_map = cv.imread(
                self.data_dir + "/smpl_nml_map/%08d.jpg" % pose_idx, cv.IMREAD_UNCHANGED
            )
            smpl_nml_map = (smpl_nml_map / 255.0).astype(np.float32)
            nml_map_size = smpl_nml_map.shape[1] // 2
            smpl_nml_map = np.concatenate(
                [smpl_nml_map[:, :nml_map_size], smpl_nml_map[:, nml_map_size:]], 2
            )
        ...
        data_item["cano2live_jnt_mats"] = torch.matmul(
            live_smpl.A[0], torch.linalg.inv(cano_smpl.A[0])
        )
        data_item["cano2live_jnt_mats_woRoot"] = torch.matmul(
            live_smpl_woRoot.A[0], torch.linalg.inv(cano_smpl.A[0])
        )
        ...
        if training:
            ...
            # If we we NeRF, we need to random sample the rays. Each item got a number
            # of rays sampled to render
            elif self.mode == "nerf":
                ...
                nerf_random = nerf_util.sample_randomly_for_nerf_rendering(
                    ...
                    mask_img,
                    depth_img,
                    self.extr_mats[view_idx],
                    self.intr_mats[view_idx],
                    live_bounds,
            ...
            data_item.update(
                {"img_h": img_h, "img_w": img_w, "extr": extr, "intr": intr}
            )
        ...
        if self.mode == "nerf" or self.mode == "3dgs" and not training:
            ...
            (
                ...
                data_item["left_live_mano_n"],
                data_item["right_live_mano_v"],
                data_item["right_live_mano_n"],
    ...
    def load_smpl_data(self):
        ...
        self.smpl_data = {
            k: torch.from_numpy(v.astype(np.float32)) for k, v in smpl_data.items()
        }
    ...
    def get_boundary_mask(mask, kernel_size=5):
        ...
        boundary_mask = np.logical_or(
            boundary_mask, np.logical_and(mask_bk > 5, mask_bk < 250)
        )
    ...
    def compute_pca(self, n_components=10):
        ...
        if not os.path.exists(
            self.data_dir + "/smpl_pos_map/pca_%d.ckpt" % n_components
        ):
            pose_conds = []
            mask = None
            for pose_idx in tqdm(self.pose_list, desc="Loading position maps..."):
                pose_map = cv.imread(
                    self.data_dir + "/smpl_pos_map/%08d.exr" % pose_idx,
                    cv.IMREAD_UNCHANGED,
                )
            ...
            joblib.dump(
                self.pca, self.data_dir + "/smpl_pos_map/pca_%d.ckpt" % n_components
            )
            ...
            pose_map = cv.imread(
                sorted(glob.glob(self.data_dir + "/smpl_pos_map/0*.exr"))[0],
                cv.IMREAD_UNCHANGED,
            )
            pose_map = pose_map[:, : pose_map.shape[1] // 2]
            self.pos_map_mask = np.linalg.norm(pose_map, axis=-1) > 1e-6

    def transform_pca(self, pose_conds, sigma_pca=2.0):
...
class MvRgbDatasetTHuman4(MvRgbDatasetBase):
    ...
    def load_cam_data(self):
        ...
        for view_idx in range(self.view_num):
            ...
            extr_mat[:3, :3] = np.array(
                cam_data["cam%02d" % view_idx]["R"], np.float32
            ).reshape(3, 3)
            extr_mat[:3, 3] = np.array(cam_data["cam%02d" % view_idx]["T"], np.float32)
            self.extr_mats.append(extr_mat)
        self.intr_mats = [
            np.array(cam_data["cam%02d" % view_idx]["K"], np.float32).reshape(3, 3)
            for view_idx in range(self.view_num)
        ]
        self.img_heights = [
            cam_data["cam%02d" % view_idx]["imgSize"][1]
            for view_idx in range(self.view_num)
        ]
        self.img_widths = [
            cam_data["cam%02d" % view_idx]["imgSize"][0]
            for view_idx in range(self.view_num)
        ]
    ...
    def filter_missing_files(self):
        ...
        for line in lines:
            line = line.replace(
                "\\", "/"
            )  # considering both Windows and Ubuntu file system
    ...
    def load_color_mask_images(self, pose_idx, view_idx):
        color_img = cv.imread(
            self.data_dir + "/images/cam%02d/%08d.jpg" % (view_idx, pose_idx),
            cv.IMREAD_UNCHANGED,
        )
        mask_img = cv.imread(
            self.data_dir + "/masks/cam%02d/%08d.jpg" % (view_idx, pose_idx),
            cv.IMREAD_UNCHANGED,
        )
...
class MvRgbDatasetAvatarReX(MvRgbDatasetBase):
    ...
    def load_cam_data(self):
        ...
        for view_idx in range(self.view_num):
            ...
            self.extr_mats.append(extr_mat)
        self.intr_mats = [
            np.array(cam_data[self.cam_names[view_idx]]["K"], np.float32).reshape(3, 3)
            for view_idx in range(self.view_num)
        ]
        self.img_heights = [
    ...
    def load_color_mask_images(self, pose_idx, view_idx):
        ...
        color_img = cv.imread(
            self.data_dir + "/%s/%08d.jpg" % (cam_name, pose_idx), cv.IMREAD_UNCHANGED
        )
        mask_img = cv.imread(
            self.data_dir + "/%s/mask/pha/%08d.jpg" % (cam_name, pose_idx),
            cv.IMREAD_UNCHANGED,
        )
        return color_img, mask_img
...
class MvRgbDatasetActorsHQ(MvRgbDatasetBase):
    ...
    def load_cam_data(self):
        ...
        img_heights = []
        with open(
            self.data_dir + "/4x/calibration.csv", "r", newline="", encoding="utf-8"
        ) as fp:
    ...
    def load_color_mask_images(self, pose_idx, view_idx):
        ...
        color_img = cv.imread(
            self.data_dir
            + "/4x/rgbs/%s/%s_rgb%06d.jpg" % (cam_name, cam_name, pose_idx),
            cv.IMREAD_UNCHANGED,
        )
        mask_img = cv.imread(
            self.data_dir
            + "/4x/masks/%s/%s_mask%06d.png" % (cam_name, cam_name, pose_idx),
            cv.IMREAD_UNCHANGED,
        )
        return color_img, mask_img
