...
class TemplateTrainer(BaseTrainer):
    ...
    def update_config_before_epoch(self, epoch_idx):
        ...
        print(
            "# Optimizable variable number in network: %d"
            % sum(p.numel() for p in self.network.parameters() if p.requires_grad)
        )

    # Does the forward pass use NeRF to render the template and calculate the loss against the
    # multiple views of the selected frame (ground truth)?
    def forward_one_pass(self, items):
        ...
        if "nerf_random" in items:
            # Get the NeRF ground truth. Apparently, each item is a dictionary
            # with information for each random sample ray for NeRF
            items.update(items["nerf_random"])
            render_output = self.network.render(
                items, depth_guided_sampling=self.opt["train"]["depth_guided_sampling"]
            )

            # color loss
            if "rgb_map" in render_output:
                color_loss = torch.nn.L1Loss()(
                    render_output["rgb_map"], items["color_gt"]
                )
                total_loss += self.loss_weight["color"] * color_loss
                batch_losses.update({"color_loss_random": color_loss.item()})

            # mask loss
            if "acc_map" in render_output:
                mask_loss = torch.nn.L1Loss()(
                    render_output["acc_map"], items["mask_gt"]
                )
                total_loss += self.loss_weight["mask"] * mask_loss
                batch_losses.update({"mask_loss_random": mask_loss.item()})

            # eikonal loss
            if "normal" in render_output:
                eikonal_loss = (
                    (torch.linalg.norm(render_output["normal"], dim=-1) - 1.0) ** 2
                ).mean()
                total_loss += self.loss_weight["eikonal"] * eikonal_loss
                batch_losses.update({"eikonal_loss": eikonal_loss.item()})
    ...
    def run(self):
        ...
        self.set_net_dict({"network": self.network})
        self.set_optm_dict(
            {"network": torch.optim.Adam(self.network.parameters(), lr=1e-3)}
        )
        # Details about learning rate, defined in the config file
        self.set_lr_schedule_dict(
            {
                "network": lr_schedule.get_learning_rate_schedules(
                    **self.opt["train"]["lr"]["network"]
                )
            }
        )
        self.set_update_keys(["network"])
        # Disable gradient for the hand networks when no need for fine tuning
        ...
        # Perceptual loss while optimizing the template if needed
        ...
        with torch.no_grad():
            self.network.eval()
            vertices, faces, normals = self.test_geometry(
                items, space="cano", testing_res=(256, 256, 128)
            )
            save_mesh_as_ply(
                self.opt["train"]["data"]["data_dir"] + "/template.ply",
                vertices,
                faces,
                normals,
            )

    # TODO: Study this function to see what it does. At a high level: generate a
    # mesh out of the template, and save it as a .ply file
    def test_geometry(self, items, space="live", testing_res=(128, 128, 128)):
        ...
        for i in range(0, vol_pts.shape[0], chunk_size):
            ...
            if space == "live":
                cano_pts_chunk, near_flag = self.network.transform_live2cano(
                    vol_pts_chunk, items, near_thres=0.1
                )
            else:
                cano_pts_chunk = vol_pts_chunk
                dists, _, _ = pytorch3d.ops.knn_points(
                    cano_pts_chunk, items["cano_smpl_v"], K=1
                )
                near_flag = dists[:, :, 0] < (0.1**2)  # (1, N)
                near_flag.fill_(True)
                if (~near_flag).sum() > 0:
                    sdf_chunk[~near_flag[0]] = (
                        self.network.cano_weight_volume.forward_sdf(
                            cano_pts_chunk[~near_flag][None]
                        )[0, :, 0]
                    )
            if near_flag.sum() > 0:
                ret = self.network.forward_cano_radiance_field(
                    cano_pts_chunk[near_flag][None], None, items
                )
                if self.network.with_hand:
                    self.network.fuse_hands(
                        ret, vol_pts_chunk[near_flag][None], None, items, space
                    )
        ...
        vertices, faces, normals = recon_util.recon_mesh(
            sdf_list, testing_res, bounds, iso_value=0.0
        )
    ...
    def mini_test(self):
        ...
        save_mesh_as_ply(
            output_dir + "/batch_%d.ply" % self.iter_idx, vertices, faces, normals
        )
